...
import type { EvidenceFile, EvidenceKind, LatestPacketResponse } from "@shared/schema";
...
export default function DisputeDetail() {
  ...
  const { data: disputeData, isLoading: disputeLoading, error: disputeError } = useQuery<{ dispute: any }>({
    queryKey: ['/api/disputes', stripeId],
    enabled: !!stripeId,
  });

  // Fetch evidence
  const { data: evidenceData, isLoading: evidenceLoading } = useQuery<{ evidence: EvidenceFile[] }>({
    queryKey: ['/api/evidence', stripeId],
    enabled: !!stripeId,
  });

  // Fetch latest packet
  const { data: packetData, isLoading: packetLoading } = useQuery<LatestPacketResponse>({
  ...
  const generatePacketMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/packets/latest', stripeId] });
      toast({
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <Card className="border-destructive">
          <CardContent className="pt-6">
            <p className="text-destructive">Error loading dispute: {disputeError.message}</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  const evidenceTemplate = dispute ? getEvidenceTemplate(dispute.reason) : { required: [], optional: [] };
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="mb-8">
        ...
        {disputeLoading ? (
          ...
          <div className="flex items-center gap-4">
            <h1 className="text-3xl font-semibold font-mono" data-testid="text-dispute-id">{stripeId}</h1>
            {dispute && (
        ...
        {disputeLoading ? (
          ...
          <>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-lg font-semibold" data-testid="text-due-date">
                  {formatDate(dispute.evidence_details?.due_by ? dispute.evidence_details.due_by * 1000 : null)}
                </p>
              </CardContent>
            </Card>
          </>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Based on the dispute reason, here's what you should provide
              </p>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Uploaded Evidence ({evidence.length})</CardTitle>
            </CardHeader>
            <CardContent>
              {evidenceLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground" data-testid="text-no-evidence">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {file.kind} • {formatBytes(file.sizeBytes)} • {formatDate(new Date(file.createdAt).getTime())}
                          </p>
                        </div>
                      </div>
        ...
        <div className="space-y-8">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                <Label htmlFor="kind" className="text-sm font-medium mb-2 block">
                  Evidence Type
                </Label>
                <Select value={selectedKind} onValueChange={(v) => setSelectedKind(v as EvidenceKind)}>
                  <SelectTrigger id="kind" data-testid="select-evidence-kind">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <CardContent className="space-y-4">
              {packetLoading ? (
                ...
                <div className="space-y-4">
                  ...
                  <a href={`/api/packets/download/${packet.id}`} download>
                    <Button variant="outline" className="w-full" data-testid="button-download-packet">
                ...
                <p className="text-sm text-muted-foreground text-center py-4" data-testid="text-no-packet">
